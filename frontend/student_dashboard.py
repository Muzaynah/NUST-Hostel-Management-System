...
class StudentDashboard(tk.Frame):
    def __init__(self, master, show_outpass, show_complaints, show_attendance):
        super().__init__(master, bg='#E0E6EE')  # Set background color to light gray
        self.master = master
    ...
    def create_widgets(self):
        ...
        left_panel = tk.Frame(self, bg='#E0E6EE')
        left_panel.pack(side=tk.LEFT, fill=tk.Y, padx=(300,0), pady=150)

        # Buttons for different sections
        button_bg_color = 'white'  # Light blue color for buttons

        # Adjusting the size and position of buttons
        Button(left_panel, text='Outpass', command=self.show_outpass, bg=button_bg_color, fg='#014a81', width=30, height=2,
               font=('Helvetica', 14)).pack(pady=(0, 20), anchor=tk.NW)  # Use anchor=tk.NW to align to the top-left corner)
        Button(left_panel, text='Complaints', command=self.show_complaints, bg=button_bg_color, fg='#014a81', width=30,
               height=2, font=('Helvetica', 14)).pack(pady=20, anchor=tk.NW)  # Use anchor=tk.NW to align to the top-left corner)
        Button(left_panel, text='Attendance', command=self.show_attendance, bg=button_bg_color, fg='#014a81', width=30,
               height=2, font=('Helvetica', 14)).pack(pady=20, anchor=tk.NW)  # Use anchor=tk.NW to align to the top-left corner)

        # Right panel for notifications
        right_panel = tk.Frame(self, bg='#014a81', width=250, height=300)
        right_panel.pack(side=tk.RIGHT, anchor=tk.N, padx=(0,300), pady=150)

        # Label for Notifications
        notification_label = Label(right_panel, text='Notifications', font=('Helvetica', 16), bg='#014a81', fg='white')
        ...
        notification_listbox = Listbox(right_panel, bg='white', fg='black', selectbackground='#1f2b38',
